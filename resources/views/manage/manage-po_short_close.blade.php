...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           <a href="{{ route('add-po_short_close') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">PO Short Close</h4>
    </div>
...
jQuery(document).ready(function() {
    ...
    jQuery('#export-excel').on('click',function(){
        jQuery('.export_po_shortclose').click();
    });
    loadDataTable();
    ...
    function loadDataTable() {
        ...
        // "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],  
            ...
            buttons:
                [
                    {
                        extend:'excel',
                        filename: 'PO Short Close',
                        title:"",
                        className: 'export_po_shortclose d-none',
                        exportOptions: {
                            columns: ':not(:eq(0))',
                            modifier: {
                                page: 'all'
        ...
        ajax: {
                url: "{{ route('listing-po_short_close') }}",
                type: "POST",
                headers: headerOpt,
                data: {
        ...
        columns: [    
            ...
            { data: 'sc_date' ,name: 'purchase_order_short_close.sc_date' , },
            ...
            { data: 'sc_qty' ,name: 'purchase_order_short_close.sc_qty'},
            { data: 'reason' ,name: 'purchase_order_short_close.reason'},
            { data: 'last_by_user_id',name: 'last_by_user_id' , },
            { data: 'last_on',name: 'purchase_order_short_close.last_on' , },
            { data: 'created_by_user_id',name: 'created_by_user_id' ,},
            { data: 'created_on',name: 'purchase_order_short_close.created_on' , },
        ],
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('delete-po_short_close') }}",
                    type: 'GET',
                    data: "id="+data["posc_id"],
                    headers: headerOpt,
                    dataType: 'json',
                    processData: false,
