...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           <a href="{{ route('add-item_issue') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Item Issue Slip</h4>
    </div>
...
jQuery(document).ready(function() {
    ...
    jQuery('#export-excel').on('click',function(){
        jQuery('.export_item_issue').click();
    });
    loadDataTable();
    ...
    function loadDataTable() {
        ...
        // "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],  
            ...
            buttons:
                    [
                        {
                            extend:'excel',
                            filename: 'Item issue Slip',
                            title:"",
                            className: 'export_item_issue d-none',
                            exportOptions: {
        ...
        columns: [
            ...
            { data: 'last_on', name: 'item_issue.last_on', },
            { data: 'created_by_user_id', name: 'created_by_user_id', },
            { data: 'created_on', name: 'item_issue.created_on', },
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('remove-item_issue') }}",
                    type: 'GET',
                    data: "id="+data["item_issue_id"],
                    headers: headerOpt,
                    dataType: 'json',
                    processData: false,
