...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("material_request","add"))
           <a href="{{ route('add-material_request') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Material Request</h4>
    </div>
        ...
        <table id="dyntable" class="table table-infinite table-bordered responsive table-autowidth">
            <thead>
                <tr class="main-header">
                    ...
                    <th calss="head0">To Location</th>
                    <th calss="head0">Cust. Group</th>                   
...
jQuery('#export-excel').on('click',function(){
    jQuery('.export_material_request').click();
});
...
function loadDataTable() {
            ...
            [
                {
                    ...
                    title:"",
                    className: 'export_material_request d-none',
                    exportOptions: {
    ...
    ajax: {
            url: "{{ route('listing-material_request') }}",
            type: "POST",
            headers: headerOpt,
            data: {
    ...
    // { data: 'sm_approval', name: 'material_request.sm_user_id', },    
    //  { data: 'zsm_approval', name: 'material_request.zsm_user_id', },    
    //  { data: 'md_approval', name: 'material_request.md_user_id', },    
        { data: 'last_by_user_id', name: 'last_by_user_id', },
        { data: 'last_on', name: 'material_request.last_on', },
        { data: 'created_by_user_id', name: 'created_by_user_id', },
        { data: 'created_on', name: 'material_request.created_on', },
...
jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
    ...
    jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
        if(r === true){
            jQuery.ajax({
				url: "{{ route('remove-material_request') }}",
				type: 'GET',
				data: "id="+data["mr_id"],
                headers: headerOpt,
                dataType: 'json',
