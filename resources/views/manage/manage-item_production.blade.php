...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("item_production","add"))
           <a href="{{ route('add-item_production') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Item Production</h4>
    </div>
        ...
        <table id="dyntable" class="table table-infinite table-bordered responsive table-autowidth">
            <thead>
                <tr class="main-header">
                    ...
                    <th class="head0">Prod. No. </th>
                    <th class="head0">Prod. Date </th>            
...
jQuery(document).ready(function() {
    ...
    function loadDataTable() {
        ...
        // "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],  
            ...
            buttons:
                    [
                        {
                            extend:'excel',
                            filename: 'Item production',
                            title:"",
                            className: 'export_item_pro d-none',
                            exportOptions: {
        ...
        ajax: {
                url: "{{ route('listing-item_production') }}",
                type: "POST",
                headers: headerOpt,
                data: {
        ...
        columns: [
            ...
            { data: 'ip_number.' ,name: 'item_production.ip_number' , },
            ...
            { data: 'last_on', name: 'item_production.last_on', },
            { data: 'created_by_user_id', name: 'created_by_user_id', },
            { data: 'created_on', name: 'item_production.created_on', },
            { data: 'ip_sequence' ,name: 'item_production.ip_sequence',  visible:false},  
        ],
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('remove-item_production') }}",
                    type: 'GET',
                    data: "id="+data["ip_id"],
                    headers: headerOpt,
                    dataType: 'json',
