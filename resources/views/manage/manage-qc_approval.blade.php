...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("qc_approval","add"))
           <a href="{{ route('add-qc_approval') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">QC Approval</h4>
    </div>
        ...
        <table id="dyntable" class="table table-infinite table-bordered responsive table-autowidth">
            <thead>
                <tr class="main-header">
                    ...
                    <th class="head1">QC Qty.</th>
                    <th class="head1">OK Qty.</th>
                    <th class="head1">Reject  Qty.</th>
                    <th class="head1">Rejection Reason</th>
    ...
    function loadDataTable() {
        ...
        buttons:
                [
                    {
                        extend:'excel',
                        filename: 'QC Approval',
                        title:"",
                        className: 'export_hsn d-none',
                        exportOptions: {
                            columns: function(idx, data, node) {
                                return idx !== 0 && idx !== 20 && table.columns(idx).visible();
                            },
                            // columns: ':not(:eq(0))',
                            modifier: {
                                page: 'all'
        ...
        columns: [
            ...
            { data: 'ok_qty', name: 'ok_qty', },
            { data: 'reject_qty', name: 'reject_qty', },
            { data: 'rejection_reason', name: 'rejection_reason', },
            { data: 'last_by_user_id', name: 'last_by_user_id', },
            { data: 'last_on', name: 'qc_approval.last_on', },
            { data: 'created_by_user_id', name: 'created_by_user_id', },
            { data: 'created_on', name: 'qc_approval.created_on', },
            { data: 'qc_sequence' ,name: 'qc_approval.qc_sequence' , visible:false},
        ],
