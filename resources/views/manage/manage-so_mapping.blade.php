...
<ul class="breadcrumbs">
    ...
    <li>Customer Replacement SO Mapping</li>
</ul>
@endsection
...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("so_mapping","add"))
           <a href="{{ route('add-so_mapping') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Customer Replacement SO Mapping</h4>
    </div>
        ...
        <table id="dyntable" class="table table-infinite table-bordered responsive table-autowidth">
            <thead>
                <tr class="main-header">
                    ...
                    <th class="head1">SO Mapping  No.</th>
                    <th class="head1">SO Mapping  Date</th>
...
jQuery(document).ready(function() {
    ...
    jQuery('#export-excel').on('click',function(){
        jQuery('.export_so_mapping').click();
    });
    loadDataTable();
...
function loadDataTable() {
    ...
    // "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],  
        ...
        buttons:
            [
                {
                    extend:'excel',
                    filename: 'Customer Replacement SO Mapping',
                    title:"",
                    className: 'export_so_mapping d-none',
                    exportOptions: {
                        columns: function(idx, data, node) {
                            return idx !== 0 && idx !== 11 && table.columns(idx).visible();
                        },
                        // columns: ':not(:eq(0))',
                        modifier: {
                            page: 'all'
    ...
    columns: [    
        ...
        { data: 'map_qty' ,name: 'so_mapping_details.map_qty'},
        { data: 'unit_name' ,name: 'units.unit_name'},
        { data: 'last_by_user_id',name: 'so_mapping.last_by_user_id' , },
        { data: 'last_on',name: 'so_mapping.last_on' , },
        { data: 'created_by_user_id',name: 'so_mapping.created_by_user_id' ,},
        { data: 'created_on',name: 'so_mapping.created_on' , },
        { data: 'so_mapping_sequence' ,name: 'so_mapping.so_mapping_sequence',  visible:false},  
    ],
...
jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
    ...
    jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
        if(r === true){
            jQuery.ajax({
                url: "{{ route('delete-so_mapping') }}",
                type: 'GET',
                data: "id="+data["mapping_id"],
                headers: headerOpt,
                dataType: 'json',
                processData: false,
