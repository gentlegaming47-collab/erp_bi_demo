...
<ul class="breadcrumbs">
    ...
    <li>Purchase Requisition Short Close</li>
</ul>
@endsection
...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("purchase_requisition_short_close","add"))
           <a href="{{ route('add-purchase_requisition_short_close') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Purchase Requisition Short Close</h4>
    </div>
...
var headerOpt = {'Authorization':'Bearer {{ Auth::user()->auth_token }}','X-CSRF-TOKEN':'{{ csrf_token() }}'};
    ...
    jQuery('#export-excel').on('click',function(){
        jQuery('.export_pr_shortclose').click();
    });
    ...
    function loadDataTable() {
        ...
        buttons:
            [
                {
                    extend:'excel',
                    filename: 'PR Short Close',
                    title:"",
                    className: 'export_pr_shortclose d-none',
                    exportOptions: {
                        columns: ':not(:eq(0))',
                        modifier: {
                            page: 'all'
        ...
        ajax: {
                url: "{{ route('listing-purchase_requisition_short_close') }}",
                type: "POST",
                headers: headerOpt,
                data: {
        ...
        columns: [    
            ...
            { data: 'pr_sc_date' ,name: 'purchase_requisition_short_close.pr_sc_date' , },
            { data: 'pr_number' ,name: 'purchase_requisition.pr_number' , },
            { data: 'pr_date' ,name: 'purchase_requisition.pr_date' , },
            { data: 'pr_form_value_fix', name: 'purchase_requisition.pr_form_value_fix', },    
            ...
            { data: 'pr_sc_qty' ,name: 'purchase_requisition_short_close.pr_sc_qty'},
            { data: 'reason' ,name: 'purchase_requisition_short_close.reason'},
            { data: 'last_by_user_id',name: 'last_by_user_id' , },
            { data: 'last_on',name: 'purchase_requisition_short_close.last_on' , },
            { data: 'created_by_user_id',name: 'created_by_user_id' ,},
            { data: 'created_on',name: 'purchase_requisition_short_close.created_on' , },
        ],
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('remove-purchase_requisition_short_close') }}",
                    type: 'GET',
                    data: "id="+data["prsc_id"],
                    headers: headerOpt,
                    dataType: 'json',
                    processData: false,
