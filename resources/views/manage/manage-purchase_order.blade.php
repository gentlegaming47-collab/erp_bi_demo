@extends('layouts.app',['pageTitle' => 'Purchase Order'])
...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           <a href="{{ route('add-purchase_order') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
...
var headerOpt = {'Authorization':'Bearer {{ Auth::user()->auth_token }}','X-CSRF-TOKEN':'{{ csrf_token() }}'};
    ...
    function loadDataTable() {
        ...
        ajax: {
                url: "{{ route('listing-purchase_order') }}",
                type: "POST",
                headers: headerOpt,
                data: {
        ...
        columns: [
            ...
            // { data: 'total_amount', name: 'purchase_order.total_amount', },
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('delete-purchase_order') }}",
                    type: 'GET',
                    data: "id="+data["po_id"],
                    headers: headerOpt,
                    dataType: 'json',
