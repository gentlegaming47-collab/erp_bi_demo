...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("grn_verification","add"))
                <a href="{{ route('add-grn_verification') }}" class="btn btn-inverse">Add</a>
           @endif
          
        </div>
        <h4 class="widgettitle">GRN Verification</h4>
    </div>
...
jQuery('#export-excel').on('click',function(){
    jQuery('.export_grn_verification').click();
});
...
jQuery(document).ready(function() {
    ...
    function loadDataTable() {
        ...
        table = jQuery('#dyntable').DataTable({
            ...
            ajax: {
                    url: "{{ route('listing-grn_verification') }}",
                    type: "POST",
                    headers: headerOpt,
                    data: {
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('delete-grn_verification') }}",
                    type: 'GET',
                    data: "id="+data["gv_id"],
                    headers: headerOpt,
                    dataType: 'json',
