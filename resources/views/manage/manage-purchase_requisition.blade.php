...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("purchase_requisition","add"))
           <a href="{{ route('add-purchase_requisition') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">Purchase Requisition</h4>
    </div>
...
var headerOpt = {'Authorization':'Bearer {{ Auth::user()->auth_token }}','X-CSRF-TOKEN':'{{ csrf_token() }}'};
    ...
    jQuery('#export-excel').on('click',function(){
        jQuery('.purchase_requisition').click();
    });
    ...
    function loadDataTable() {
            ...
            [
                {
                    ...
                    title:"",
                    className: 'purchase_requisition d-none',
                    exportOptions: {
        ...
        ajax: {
                url: "{{ route('listing-purchase_requisition') }}",
                type: "POST",
                headers: headerOpt,
                data: {
        ...
        columns: [
            ...
            { data: 'last_on', name: 'purchase_requisition.last_on', },
            { data: 'created_by_user_id', name: 'created_by_user_id', },
            { data: 'created_on', name: 'purchase_requisition.created_on', },
    ...
    jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
        ...
        jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
            if(r === true){
                jQuery.ajax({
                    url: "{{ route('remove-purchase_requisition') }}",
                    type: 'GET',
                    data: "id="+data["pr_id"],
                    headers: headerOpt,
                    dataType: 'json',
