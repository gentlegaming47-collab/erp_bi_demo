...
<div class="widgetbox">
    <div class="headtitle">
        <div class="btn-group">
           ...
           @if(hasAccess("transaction_so_short_close","add"))
           <a href="{{ route('add-transaction_so_short_close') }}" class="btn btn-inverse">Add</a>
           @endif
        </div>
        <h4 class="widgettitle">SO Short Close</h4>
    </div>
...
jQuery('#export-excel').on('click',function(){
    jQuery('.export_transaction_so_short_close').click();
});
function loadDataTable() {
    ...
    buttons:
        [
            {
                extend:'excel',
                filename: 'SO Short Close',
                title:"",
                className: 'export_transaction_so_short_close d-none',
                exportOptions: {
                    columns: ':not(:eq(0))',
                    modifier: {
                        page: 'all'
    ...
    ajax: {
            url: "{{ route('listing-transaction_so_short_close') }}",
            type: "POST",
            headers: headerOpt,
            data: {
    ...
    columns: [    
        ...
        { data: 'tr_sc_date' ,name: 'transaction_so_short_close.tr_sc_date' , },
        ...
        { data: 'tr_sc_qty' ,name: 'transaction_so_short_close.tr_sc_qty'},
        { data: 'reason' ,name: 'transaction_so_short_close.reason'},
        { data: 'last_by_user_id',name: 'last_by_user_id' , },
        { data: 'last_on',name: 'transaction_so_short_close.last_on' , },
        { data: 'created_by_user_id',name: 'created_by_user_id' ,},
        { data: 'created_on',name: 'transaction_so_short_close.created_on' , },
    ],
...
jQuery('#dyntable tbody').on( 'click', '#del_a', function () {
    ...
    jConfirm('Are You Sure, You Want <lw-c>to</lw-c> Delete ?', 'Confirmation', function(r) {
        if(r === true){
            jQuery.ajax({
                url: "{{ route('delete-transaction_so_short_close') }}",
                type: 'GET',
                data: "id="+data["tr_sosc_id"],
                headers: headerOpt,
                dataType: 'json',
                processData: false,
