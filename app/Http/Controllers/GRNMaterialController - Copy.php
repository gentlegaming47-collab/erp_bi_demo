                ...
                foreach ($request->item_id as $ctKey => $ctVal )
                {
                    if($request->grn_type_fix_id == '3'){
                        if ($ctVal != null) {
                            ...
                            // if($request->grn_type_fix_id == '3' && isset($request->dp_details_id[$ctKey]) && $request->dp_details_id[$ctKey] != 0){

                            //     // $dcQtySum = DispatchPlanDetails::where('dp_details_id',$request->dp_details_id[$ctKey])->sum('plan_qty');
                            //     $dcQtySum = LoadingEntryDetails::where('dp_details_id',$request->dp_details_id[$ctKey])->sum('loading_qty');
                            ...
                            $loading_qty = LoadingEntryDetails::where('dp_details_id',$request->dp_details_id[$ctKey])->sum('loading_qty');
                            $check_grn = GRNMaterialDetails::where('dc_details_id',$request->dp_details_id[$ctKey])->where('item_id',$ctVal)->first();

                            if(!empty($check_grn) && $check_grn != null){
                                $old_qty = floatval($check_grn->grn_qty ?? 0);
                                $new_qty = floatval($request->grn_qty[$ctKey] ?? 0);
                                $rate_unit = floatval($request->rate_unit[$ctKey] ?? 0);
                                $grn_qty = $old_qty + $new_qty;

                                $loading_qty_mismatch = $loading_qty - $grn_qty;

                                $up_gen_material_details =  GRNMaterialDetails::where('dc_details_id',$request->dp_details_id[$ctKey])->update([
                                    'grn_qty'  => $grn_qty,
                                    'mismatch_qty'  => $loading_qty_mismatch,
                                    'remarks'  => null,
                                    'rate_per_unit' => isset($request->rate_unit[$ctKey])     ? $request->rate_unit[$ctKey]     : null,
                                    'amount' => $grn_qty * $rate_unit,
                                ...
                                if(empty($request->item_details_id[$ctKey]) && $request->item_details_id[$ctKey] == 0){
                                    if(isset($request->qc_required[$ctKey]) &&  $request->qc_required[$ctKey] == 'No' &&  $request->service_item[$ctKey] == 'No'){
                                        stockEffect($locationCode->id,$ctVal,$request->pre_item[$ctKey],$request->grn_qty[$ctKey],0,'add','U','GRN',$check_grn->grn_details_id);
                                    }
                                }
                            }else{
...
// dd($e->getMessage(),$e->getLine());
    ...
    // public function edit(GRNMaterial $grnMaterial, $id)
    {
        ...
        if ($grnMaterialDetails != null) {
            ...
            if($grnMaterialDetails != null){
                $grnMaterialDetails = $grnMaterialDetails->filter(function ($item) use ($grnMaterial) {
                    ...
                    // }
                     ...
                     if($grnMaterial->grn_type_id_fix  == 3){                     
                        ...
                        if ($isFound || $isGRNVarificationFound->isNotEmpty()) {
                            $item->in_use = true;
                        } else {
                            $item->in_use = false;
                        }
                                ...
                                if (isset($request->grn_details_id) && !empty($request->grn_details_id)) {
                                    foreach ($request->grn_details_id as $ctKey => $ctVal) { 
                                        ...
                                        if($ctVal == "0"){    
                                            ...
                                            $check_grn = GRNMaterialDetails::where('item_id',$request->item_id[$ctKey])->where('grn_id',$request->id)->where('grn_details_type',$request->grn_details_type[$ctKey])->first();
                                            ...
                                            if(!empty($check_grn) && $check_grn != null){
                                                ...
                                                $loading_qty_mismatch = $old_qty - $grn_qty;
                                                ...
                                                if(empty($request->item_details_id[$ctKey]) && $request->item_details_id[$ctKey] == 0){
                                                    if(isset($request->qc_required[$ctKey]) && $request->qc_required[$ctKey] == 'No' && $request->service_item[$ctKey] == 'No'){
                                                        stockEffect($locationCode->id,$request->item_id[$ctKey],$request->item_id[$ctKey],$request->grn_qty[$ctKey],0,'add','U','GRN',$check_grn->grn_details_id);
                                                    }
                                                }


                                                $second_grndetail_id = $check_grn->grn_details_id;
                                            }else{
                                                $gen_material_details =  GRNMaterialDetails::create([
                                                    'grn_id'        => $request->id,
                                                    'po_details_id' => isset($request->po_details_id[$ctKey]) ? $request->po_details_id[$ctKey] : null,
                                            ...
                                            if(!empty($request->item_details_id[$ctKey]) &&  $request->item_details_id[$ctKey] != 0 ){
                                                ...
                                                // if(!empty($request->dp_details_id[$ctKey]) && $request->dp_details_id[$ctKey]  != 0 && !empty($request->item_details_id[$ctKey]) &&  $request->item_details_id[$ctKey] != 0){
                                                //   $check_grn = GRNMaterialDetails::where('grn_details_id',$ctVal)->where('dc_details_id',$request->dp_details_id[$ctKey])->first();
                                                // }elseif( !empty($request->item_details_id[$ctKey]) &&  $request->item_details_id[$ctKey] != 0){
                                                //   $check_grn = GRNMaterialDetails::where('grn_details_id',$ctVal)->first();
                                                ...
                                                // if(!empty($check_grn) && $check_grn != null && !empty($request->item_details_id[$ctKey]) &&  $request->item_details_id[$ctKey] != 0 ){
                                                //     $old_qty = floatval($check_grn->grn_qty ?? 0);
                                                //     $new_qty = floatval($request->grn_qty[$ctKey] ?? 0);
                                                //     $rate_unit = floatval($request->rate_unit[$ctKey] ?? 0);
                                                //     $grn_qty = $old_qty - $new_qty;

                                                //     if(isset($request->rate_unit[$ctKey]) && $request->dp_details_id[$ctKey] != null ){
                                                //         $loading_qty_mismatch = $loading_qty + $grn_qty;
                                                //     }else{
                                                //         $loading_qty_mismatch = $old_qty + $grn_qty;
                                                //     }

                                                //     $up_gen_material_details =  GRNMaterialDetails::where('grn_details_id',$ctVal)->where('item_id',$request->item_id[$ctKey])->update([
                                                //         'grn_qty'  => $grn_qty,
                                                //         'mismatch_qty'  => $loading_qty_mismatch,
                                                //         'remarks'  => null,
                                                //         'rate_per_unit' => isset($request->rate_unit[$ctKey])     ? $request->rate_unit[$ctKey]     : null,
...
// dd($po_data);
    ...
    public function getDcPartDataForGrn(Request $request){
        ...
        'units.unit_name', 'item_groups.item_group_name', 'units.unit_name','items.item_name' ,'dispatch_plan_details.item_id','items.item_code','sales_order_details.rate_per_unit','items.qc_required' ,'items.service_item' ,'item_details.secondary_item_name','loading_entry_details.loading_qty','loading_entry_secondary_details.plan_qty','item_details.item_details_id','loading_entry_secondary_details.le_secondary_details_id',
