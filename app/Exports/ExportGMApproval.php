<?php
    ...
    class ExportGMApproval implements FromCollection, WithHeadings, WithStyles, ShouldAutoSize
    {
        use Exportable;
        ...
        public function collection()
        {
            $query = MaterialRequest::query()
                ->select([
                    DB::raw('DATE_FORMAT(material_request.gm_approvaldate, "%d/%m/%Y") as gm_approvaldate'),
                ...
                ->join('admin','admin.id','=','material_request.gm_user_id')
                ...
                ->where('material_request.gm_user_id','=', Auth::user()->id)
                ->orderBy('material_request.mr_date', 'desc')
                ->orderBy('material_request.gm_approvaldate', 'desc');
            ...
            if(!empty($this->searchData['trans_from_date']))
            {
                try
                {
                    ...
                    $query->whereDate('material_request.gm_approvaldate', '>=', $date);
                }
                catch(\Exception $e)
                {
                ...
                $query->where(function ($q) use ($keywords) {
                    foreach ($keywords as $word) {
                        $q->where(function ($subQ) use ($word) {
                            $subQ->whereRaw('DATE_FORMAT(material_request.gm_approvaldate, "%d/%m/%Y") LIKE ?', ['%' . $word . '%'])
            ...
            {
                $mappings = [
                    1 => 'material_request.gm_approvaldate',
                ...
                foreach($this->searchData['columns'] as $idx => $cval)
                {
                    if($cval !== '')
                    {
                        $query->where(function ($q) use ($cval) {
                            $q->whereRaw('DATE_FORMAT(material_request.gm_approvaldate, "%d/%m/%Y") LIKE ?', ['%' . $cval . '%'])
                        ...
                        {
                            if(in_array($mappings[$idx], ['material_request.gm_approvaldate', 'material_request.mr_date']))
                            {
