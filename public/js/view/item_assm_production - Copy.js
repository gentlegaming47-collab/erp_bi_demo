...
function addItemAPDetails(item_mapping) {
    ...
    for (let key in item_mapping) {
        ...
        if (mapQty != undefined && mapQty != '' && stock_qty != undefined && stock_qty != '') {            
            ...
            if (!isNaN(qty)) {
                // set ih hidden variable
               
                jQuery("#assmbly_production").val(qty);

                // jQuery("#commonItemAssmProductionForm").find("#mininum_qty").val(parseFloat(qty).toFixed(3));
                // jQuery("#commonItemAssmProductionForm").find("#assembly_qty").val(parseFloat(qty).toFixed(3));

                assArray.push(qty);                
            } else {              
                jQuery("#assembly_qty").val(0);
            }
           ...
           jQuery("#mininum_qty").val(minValue);
            ...
            jQuery("#assembly_qty").val(0);
            jQuery("#commonItemAssmProductionForm").find("#mininum_qty").val(0);
        }
        ...
        ${item_name}
        <input type="hidden" name="item_id[]" id="item_id" style="width:100px" class="form-control salesmanageTable POaddtables item_id add_item item_id" tabindex="-1" value="${item_id}" readonly/></td>
...
function fillitemIProductionTable(itemAssmProductionDetails) {
    ...
    if (itemAssmProductionDetails.length > 0) {
        ...
        for (let key in itemAssmProductionDetails) {
            ...
            var item_name = itemAssmProductionDetails[key].item_name ? itemAssmProductionDetails[key].item_name : "";
            ...
            var item_group_name = itemAssmProductionDetails[key].item_group_name ? itemAssmProductionDetails[key].item_group_name : "";
           

            var stock_qty = itemAssmProductionDetails[key].stock_qty != 0 ? parseFloat(itemAssmProductionDetails[key].stock_qty).toFixed(3) : 0;
            ...
            var totalStockQty = itemAssmProductionDetails[key].stock_qty != "" || itemAssmProductionDetails[key].consumption_qty !="" ? itemAssmProductionDetails[key].stock_qty + itemAssmProductionDetails[key].consumption_qty : "";
