...
function validateEmail(e) {
    ...
    if (email != '' && email != null && emailReg.test(email)) {
        ...
        jQuery("#popup_ok").click(function () {
            setTimeout(() => {
                jQuery("#commonTransporterForm").find('#contact_person_email_id').closest('.control-group').addClass('error');
                jQuery("#commonTransporterForm").find('#contact_person_email_id').focus();
            }, 100);
        });
    }
}
...
function validateGSTIN(e) {
    ...
    if (gstin != '' && gstin != null && GSTIINregexp.test(gstin)) {
        jQuery("#commonTransporterForm").find('#gstin').removeClass('error');
        return true;
    } else {
        jAlert('Please Enter Valid Email ID');
        jQuery("#popup_ok").click(function () {
            setTimeout(() => {
                jQuery("#commonTransporterForm").find('#gstin').addClass('error');
                jQuery("#commonTransporterForm").find('#gstin').focus();
            }, 100);
        });
    }
}
